...
issue (see the lifecycle in `.claude/rules/github.md`). A sentence in `docs/`
that would need rewording the moment a PR merges is a sentence that belongs
in an issue instead.
...
## Two of these are contracts

`docs/SCHEMA.md` and `docs/API.md` define the database shape and the HTTP
interface. Other code — and other agents — rely on them being accurate. They
are not notes.
...
- `docs/DECISIONS.md` — append-only. A decision gets a D-number, a
  ...
  Annotate a superseded entry rather than rewriting it.
...
- `docs/REVIEW.md` — what to look for in a diff, i.e. the failures that are
  silent.
- `docs/DEPLOYMENT.md` — what has to be true to ship it.
