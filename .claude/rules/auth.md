...
`APP_AUTH_MODE` (`lib/auth-mode.ts`), which defaults to `dev` outside
production and `password` in production — so shipping cannot turn the gate
off by omission (D-006).
...
| `password` | shared `APP_PASSWORD` → HMAC token naming one user           | the only working non-local path today; token in `localStorage`         |
...
and `lib/queries/isolation.test.ts` is its executable form (D-004).
...
people apart. That ambiguity is what external OAuth exists to resolve.
...
the whole import graph before adding a dependency here (D-007).
...
Tokens carry a version. Bumping it logs everyone out, which is the honest
answer to a token whose claims no longer mean what the code assumes.

## Implementing `supabase` mode

`lib/auth-supabase.ts` is the one file that has to be written, and it
documents the four steps. Everything else already speaks in terms of a
`users` row: `lib/session.ts` asks that module who the caller is, maps the
answer through `users.supabase_user_id`, provisions a row on first sign-in,
and hands the rest of the app a local user id. No query, route or component
changes.

Use `jose` for JWT verification — it is Edge-compatible, which
`jsonwebtoken` is not, and this runs in the middleware graph.

## Client side

`lib/auth-client.ts` owns the token: `getToken`/`setToken`/`clearToken`, and
`authedFetch`, which attaches `x-app-token` and — on a 401 — clears the
