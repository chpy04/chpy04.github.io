...
gh pr checks <pr>
gh pr diff <pr>
gh api "repos/$(gh repo view --json nameWithOwner -q .nameWithOwner)/pulls/<pr>/comments"  # inline review comments
...
`tsc` passed is usually an App Router export signature, or something hoisted
to module scope that needs a database; `round-trip` failing means
`scripts/smoke.ts` no longer holds, which is the project's acceptance bar and
...
your change touched. **0 skipped** — a non-zero skip count means the
integration suites self-skipped and the green is on a subset.
