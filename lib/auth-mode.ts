...
export function getAuthMode(): AuthMode {
  const raw = process.env.APP_AUTH_MODE?.trim().toLowerCase();

  if (raw && raw.length > 0) {
    if (!MODES.includes(raw as AuthMode)) {
      throw new Error(`APP_AUTH_MODE="${raw}" is not one of: ${MODES.join(', ')}`);
    }
 ...
 *  unless `APP_AUTH_MODE=dev` was set there deliberately. */
