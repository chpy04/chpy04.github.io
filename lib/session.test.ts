/**
 * Session resolution across all three auth modes.
 *
 ...
 * own server process with a different `APP_AUTH_MODE`, and two
...
test('dev mode resolves the seeded user with no token at all', { skip }, async () => {
  ...
  const user = await withEnv({ APP_AUTH_MODE: 'dev' }, () => requireUser(request()));
...
test('dev mode ignores whatever token happens to be lying around', { skip }, async () => {
  ...
  const expected = await fixtures.seedUserId();
  const user = await withEnv({ APP_AUTH_MODE: 'dev' }, () => requireUser(request('garbage.token')));
  assert.equal(user.id, expected);
});
...
test('password mode rejects a request with no token', { skip }, async () => {
  await assert.rejects(
    () => withEnv({ APP_AUTH_MODE: 'password', AUTH_SECRET: SECRET }, () => requireUser(request())),
    UnauthorizedError,
  );
});
...
test('password mode resolves the user named by the token', { skip }, async () => {
  ...
  const resolved = await withEnv({ APP_AUTH_MODE: 'password', AUTH_SECRET: SECRET }, () =>
...
test(
  ...
  async () => {
    ...
    await assert.rejects(
      () =>
        withEnv({ APP_AUTH_MODE: 'password', AUTH_SECRET: SECRET }, () =>
          requireUser(request(token)),
        ),
      UnauthorizedError,
    );
...
test(
  ...
  async () => {
    ...
    await assert.rejects(
      () => withEnv({ APP_AUTH_MODE: 'supabase' }, () => requireUser(request('anything'))),
...
test('loginWithPassword rejects the wrong password', { skip }, async () => {
  const token = await withEnv(
    { APP_AUTH_MODE: 'password', APP_PASSWORD: PASSWORD, AUTH_SECRET: SECRET },
