 ...
 * (a dangling id, an operation the row does not allow) — maps to 400. */
 ...
 * Distinct from `NotFoundError` on purpose: asking for another user's row
 * must look exactly like asking for one that does not exist, so ownership
 * is never leaked by the status code (see `lib/session.ts`). */
