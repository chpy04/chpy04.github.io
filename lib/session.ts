 ...
 * `npm run db:seed` created. No login screen, no token, no password.
 */
 ...
 * ambiguity is exactly what external auth resolves.
 */
  ...
  if (total > 1) {
    console.error(
      `[auth] ${total} users exist but OWNER_EMAIL is unset, so the password gate cannot tell ` +
        'which one to log in as. Set OWNER_EMAIL, or switch to APP_AUTH_MODE=supabase.',
    );
    return null;
  }
