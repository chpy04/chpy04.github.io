...
/**
 * Markdown body. The description is inserted verbatim rather than escaped:
 * a reporter pasting a code fence, a stack trace or a snippet of markup
 * means the thing they typed, and escaping it turns a useful report into an
 * unreadable one. The tradeoff is that whoever can reach this endpoint can
 * write arbitrary markdown into an issue — which is why the route resolves
 * a caller first. Revisit that call if you ever open feedback to the public.
 *
...
/**
 * `https://example.app/items/abc?tab=notes` → `/items/abc?tab=notes`.
