...
test('uploading onto an existing branch commits on top of its head', async () => {
  ...
  try {
    ...
    assert.equal(url, 'https://raw.githubusercontent.com/owner/repo/commit1/screenshots/a.png');
  } finally {
    restore();
  }
...
test('the first ever upload creates the assets branch as an orphan', async () => {
  ...
  const ref = calls.find((c) => c.path === '/repos/owner/repo/git/refs')!;
...
test('creating an issue returns its number and html url', async () => {
  const { calls, restore } = stubFetch(() => ({
    status: 201,
    body: { number: 42, html_url: 'https://github.com/owner/repo/issues/42' },
  }));

  try {
    const issue = await createIssue(CONFIG, { title: 't', body: 'b', labels: ['feedback', 'bug'] });
    assert.deepEqual(issue, { number: 42, url: 'https://github.com/owner/repo/issues/42' });
  } finally {
    restore();
  }

  assert.equal(calls[0]!.path, '/repos/owner/repo/issues');
