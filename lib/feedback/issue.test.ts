...
test('the body leads with the type and the route, without the host', () => {
  const body = issueBody({
    ...
    url: 'https://example.app/items/abc?tab=notes#n2',
    submittedAt: AT,
  });
  const lines = body.split('\n');
  assert.equal(lines[0], '**Type:** Bug report');
  assert.equal(lines[1], '**Page:** `/items/abc?tab=notes#n2`');
...
test('the origin goes in the environment block, so localhost and prod are told apart', () => {
  const fromProd = issueBody({
    kind: 'bug',
    description: 'x',
    url: 'https://example.app/items/abc',
    submittedAt: AT,
  });
  const fromLaptop = issueBody({
    kind: 'bug',
    description: 'x',
    url: 'http://localhost:3000/items/abc',
    submittedAt: AT,
  });

  assert.ok(fromProd.includes('- Origin: https://example.app'));
  assert.ok(fromLaptop.includes('- Origin: http://localhost:3000'));
  // Same page either way — which is the point of splitting them.
  assert.ok(fromProd.includes('**Page:** `/items/abc`'));
  assert.ok(fromLaptop.includes('**Page:** `/items/abc`'));
});
...
test('a backtick cannot escape the page code span', () => {
  ...
  const parsed = issueBody({
    kind: 'bug',
    description: 'x',
    url: 'https://example.app/items/a`b',
    submittedAt: AT,
  });
  assert.ok(parsed.includes('**Page:** `/items/a%60b`'));
...
test('the environment block carries the submission context', () => {
  ...
  assert.ok(body.includes('- Origin: https://example.app'));
...
test('a missing user agent says so in both forms rather than rendering empty', () => {
  const body = issueBody({ kind: 'bug', description: 'x', url: 'https://example.app/' });
