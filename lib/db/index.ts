...
declare global {
  ...
  var __appDb: PostgresJsDatabase<typeof schema> | undefined;
}

function getSql(): Sql {
  if (globalThis.__appSql) return globalThis.__appSql;
  ...
  globalThis.__appSql = client;
  return client;
}

function getDb(): PostgresJsDatabase<typeof schema> {
  if (globalThis.__appDb) return globalThis.__appDb;
  const instance = drizzle(getSql(), { schema });
  globalThis.__appDb = instance;
  return instance;
}
