 ...
 *      `x-app-token` header (swap the value for Supabase's JWT) or move
...
export class SupabaseAuthNotConfiguredError extends Error {
  constructor() {
    super(
      'APP_AUTH_MODE=supabase, but Supabase OAuth is not implemented yet — ' +
        'see lib/auth-supabase.ts. Use APP_AUTH_MODE=password or =dev.',
    );
